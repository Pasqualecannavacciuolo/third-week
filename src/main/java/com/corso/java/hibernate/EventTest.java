...
public class EventTest {
    ...
    public void testBasicUsage() {
        ...
        session.save(new Event("Un Evento seguente", "Descrizione personalizzata", new Date()));
        session.getTransaction().commit();
        session.close();

        session = sessionFactory.openSession();
        session.beginTransaction();
        List<Event> result = session.createQuery("from com.corso.java.hibernate.Event").list();
